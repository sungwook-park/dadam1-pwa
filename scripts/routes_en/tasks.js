...
function loadTasks(mode = 'incomplete') {
...
function markDone(id) {
  updateDoc(doc(db, 'tasks', id), { done: true }).then(() => loadTasks('incomplete'));
}
...
function deleteTask(id) {
  ...
  getDoc(ref).then(snap => {
    ...
    if (currentUserRole === 'admin') {
      deleteDoc(ref).then(() => loadTasks('incomplete'));
    } else {
      const deletedBy = data.deletedBy || [];
      if (!deletedBy.includes(currentUserName)) {
        deletedBy.push(currentUserName);
        updateDoc(ref, { deletedBy }).then(() => loadTasks('incomplete'));
      }
    }
  });
}
