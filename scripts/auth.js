...
function updateTitle(email) {
  ...
  if (isAdmin(email)) {
    ...
    if (pageTitle) {
      pageTitle.textContent = '다담업무';
    }
  }
}

// 권한에 따른 UI 표시/숨김
function setupUserInterface(email) {
  ...
  if (isAdmin(email)) {
    ...
    console.log('👷 작업자 모드 활성화');
  }
}

// 작업자 전용 인터페이스 표시
function showWorkerInterface() {
