...
// CSS 스타일 추가 (작업자 수정 폼용 + 관리자 통계용 - 서브타이틀 크기 조정됨)
...
window.isCurrentUserAdmin = isCurrentUserAdmin;// scripts/task-ui.js - 완전한 파일 (통계 수정 및 서브타이틀 크기 조정 완료)
...
window.loadTodayTasks = async function() {
  ...
  try {
    ...
    const allTodaySnapshot = await getDocs(allTodayQuery);

    let allTodayTasks = [];
    let allPendingTasks = [];
    let allCompletedTasks = [];

    allTodaySnapshot.forEach(docu => {
      const taskData = docu.data();
      const task = {
        id: docu.id,
        ...taskData
    ...
    console.log('📊 통계 확인:');
    console.log('전체 작업:', allTodayTasks.length);
    console.log('완료 작업:', allCompletedTasks.length);
    console.log('미완료 작업:', allPendingTasks.length);
    ...
    if (isCurrentUserAdmin()) {
      ...
      console.log('실제 표시할 미완료 작업:', allPendingTasks.length);
...
// 완료 작업 로드 - 오늘 완료된 작업만 기본 표시 (통계 수정됨)
window.loadDoneTasks = async function() {
  ...
  try {
    ...
    if (isCurrentUserAdmin()) {
      ...
      console.log('실제 표시할 완료 작업:', completedTasks.length);
